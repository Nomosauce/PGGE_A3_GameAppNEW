...
public class FlockBehaviour : MonoBehaviour
{
    ...
    void AddBoidFromPool(float x, float y, Flock flock)
    {
        GameObject boidObj = objectPooler.SpawnFromPool("Boid", new Vector3(x, y, 0.0f), Quaternion.identity);

        if (boidObj != null)
        {
            Autonomous boid = boidObj.GetComponent<Autonomous>();
            flock.mAutonomous.Add(boid);

            // Additional setup for the boid if needed
            boid.MaxSpeed = flock.maxSpeed;
            boid.RotationSpeed = flock.maxRotationSpeed;
        }
    }
